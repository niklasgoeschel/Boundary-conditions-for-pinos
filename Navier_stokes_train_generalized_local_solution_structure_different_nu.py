...
for i in range(epochs):
    ...
    for j in range(8):
        ...
        pred_u = w_1 * g_1[..., 0] + w_3 * ( Psi_u_3[:, 1:-1, 1:-1] + phi_3_tilde * (Psi_u_3_x - Psi_p_3[:, 1:-1, 1:-1]/sqrt_nu[:,1:-1,1:-1]) ) + phi_1 * phi_2 * phi_3 * phi_4 * phi_S * Psi_u
...
with open('boundary-conditions-for-pinos-code/results_Navier_stokes_different_nu/generalized_local_solution_structure/losses_errors.npy', 'wb') as f:
    np.save(f, np.array(losses))
    np.save(f, np.array(errors_u))
    np.save(f, np.array(errors_v))
    np.save(f, np.array(errors_p))

with open('boundary-conditions-for-pinos-code/results_Navier_stokes_different_nu/generalized_local_solution_structure/runtime.npy', 'wb') as f:
    np.save(f, end_time-start_time)

torch.save(model, 'boundary-conditions-for-pinos-code/results_Navier_stokes_different_nu/generalized_local_solution_structure/net.pt')