...
interior = torch.ones((res_size_x,res_size_y), dtype=torch.double).cuda()
...
with open('boundary-conditions-for-pinos-code/Navier_stokes_coeffs_nu.npy', 'rb') as f:
    nus = torch.tensor(np.load(f)[0:80], dtype=torch.double).cuda()
...
ones_y = torch.ones( (res_size_y-2), dtype=torch.double ).cuda()
...
for i in range(epochs):
    ...
    for j in range(8):
        ...
        pred_v_x_3 = ones_y*(pred_v[:,-1,2:-2] - pred_v[:,-3,2:-2]) / dx / 2
        ...
        loss_pde = (torch.nn.functional.mse_loss(res_1, torch.zeros(res_1.shape, dtype=torch.double).cuda()) + torch.nn.functional.mse_loss(res_2, torch.zeros(res_2.shape, dtype=torch.double).cuda()) + torch.nn.functional.mse_loss(res_3, torch.zeros(res_3.shape, dtype=torch.double).cuda()))
        loss_bc = torch.nn.functional.mse_loss(sqrt_nu[:, -2, 2:-2]*pred_u_x_3 - pred_p[:,-1,1:-1], torch.zeros(pred_u_x_3.shape, dtype=torch.double).cuda()) + torch.nn.functional.mse_loss(pred_v_x_3, torch.zeros(pred_v_x_3.shape, dtype=torch.double).cuda())
...
with open('boundary-conditions-for-pinos-code/results_Navier_stokes_different_nu/semi_weak/losses_errors.npy', 'wb') as f:
    np.save(f, np.array(losses))
    np.save(f, np.array(errors_u))
    np.save(f, np.array(errors_v))
    np.save(f, np.array(errors_p))

with open('boundary-conditions-for-pinos-code/results_Navier_stokes_different_nu/semi_weak/runtime.npy', 'wb') as f:
    np.save(f, end_time-start_time)

torch.save(model, 'boundary-conditions-for-pinos-code/results_Navier_stokes_different_nu/semi_weak/net.pt')