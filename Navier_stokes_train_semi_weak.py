...
for i in range(epochs):
    ...
    loss_bc = torch.nn.functional.mse_loss(sqrt_nu*pred_u_x_3 - pred_p[:,-1,1:-1], torch.zeros(pred_u_x_3.shape).cuda()) + torch.nn.functional.mse_loss(pred_v_x_3, torch.zeros(pred_v_x_3.shape).cuda())
...
with open('boundary-conditions-for-pinos-code/results_Navier_stokes/semi_weak/losses_errors.npy', 'wb') as f:
...
with open('boundary-conditions-for-pinos-code/results_Navier_stokes/semi_weak/prediction_441_83_3.npy', 'wb') as f:
    np.save(f, predicted_solution)

with open('boundary-conditions-for-pinos-code/results_Navier_stokes/semi_weak/runtime.npy', 'wb') as f:
    np.save(f, end_time-start_time)

torch.save(model, 'boundary-conditions-for-pinos-code/results_Navier_stokes/semi_weak/net.pt')